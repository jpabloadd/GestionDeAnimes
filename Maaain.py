...
FICHERO_LOG = "historial_acciones.txt"
API_URL = "https://api.jikan.moe/v4/anime"

# --- NUEVA FUNCIÓN PARA LIMPIAR PANTALLA ---
def limpiar_pantalla():
    # Detecta el sistema operativo (Windows usa 'cls', Linux/Mac usa 'clear')
    if os.name == 'nt':
        os.system('cls')
    else:
        os.system('clear')
...
# lee el fichero JSON y devuelve la lista 
def cargar_datos():
    ...
    try:
        ...
        if respuesta.status_code == 200:
            ...
            if datos_api['data']:
                ...
                if capitulos is None: capitulos = 0
                ...
                return {
                    ...
                    "capitulos": capitulos,
                    "imagen": imagen,
                    "sinopsis": item.get('synopsis', '')[:100] + "..."
                }
        return None
...
def guardar_anime():
    ...
    if resultado_api:
        ...
        if confirmar == 's':
            ...
            guardar_datos(datos)
            registrar_log(f"AÑADIDO: {nuevo_anime['nombre']} entró a la lista de espera.")
            print(f"\n ✅ ¡Guardado en 'animes.txt' y enviado a LISTA DE ESPERA (ID {nuevo_anime['id']})!")
        else:
            print(" ❌ Cancelado.")
    else:
        ...
        if opcion_manual == 's':
            ...
            datos.append(nuevo_anime)
            guardar_datos(datos)
            registrar_log(f"AÑADIDO MANUAL: {nuevo_anime['nombre']} entró a la lista de espera.") 
            ...
            while True:
                try:
                    ...
                    if 1 <= nota <= 10:
                        ...
                        if anime.get('en_lista_espera') is True:
                            anime['en_lista_espera'] = False
                            print(" 📉 El anime ha sido eliminado de la Lista de Espera.")
                            registrar_log(f"FINALIZADO: {anime['nombre']} puntuado con {nota} y sacado de espera.") # Log
                        else:
                            registrar_log(f"PUNTUADO: {anime['nombre']} actualizado con nota {nota}.")
...
def gestionar_lista_espera():
    while True:
        ...
        datos = cargar_datos()
        if not datos: 
            input("No hay datos. Pulsa ENTER.")
            continue

        if opcion == "1":
            ver_catalogo(datos) 
            try:
                id_buscado = int(input("\n » ID del anime a AÑADIR manualmente: "))
                for anime in datos:
                    if anime['id'] == id_buscado:
                        anime['en_lista_espera'] = True
                        print(f" ✅ Añadido a espera.")
                        registrar_log(f"MANUAL: {anime['nombre']} movido a lista de espera.") 
                        guardar_datos(datos)
                        break
                ...
                try:
                    ...
                    for anime in datos:
                        if anime['id'] == id_buscado and anime['en_lista_espera']:
                            ...
                            registrar_log(f"MANUAL: {anime['nombre']} eliminado de lista de espera.") 
                            guardar_datos(datos)
                            break
                except ValueError: pass
            else:
        ...
        # Pausa antes de recargar el submenú y limpiar pantalla
...
def menu_principal():
    while True:
        limpiar_pantalla() # Limpia siempre al volver al menú principal
        print("\n")
        print(" ╔════════════════════════════════════════════════════╗")
        print(" ║                 GESTOR DE ANIMES                   ║")
        print(" ╠════════════════════════════════════════════════════╣")
        print(" ║  1. 🌐  BUSCAR y Guardar                           ║")
        print(" ║  2. 📋  Ver catálogo completo                      ║")
        print(" ║  3. ⭐  Puntuar anime                              ║")
        print(" ║  4. 📌  Gestión Manual Lista de Espera             ║")
        ...
        elif opcion == "4": gestionar_lista_espera() # Este tiene su propio bucle de limpieza dentro
